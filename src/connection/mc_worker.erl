...
init(Options) ->
  Proxy = proplists:get_value( proxy, Options, false ),
  init_local( Proxy, Options ).

init_local( false, Options ) ->
...
init_local( true, Options ) ->
  {ok, Socket} = mc_auth:connect_to_database(Options),
  ConnState = form_state(Options),
  try_register(Options),
  mc_auth:auth(Socket, Options, ConnState#conn_state.database),
  {ok, #state{socket = Socket, conn_state = ConnState} }.
